...
def agroup_properties_in_sameas(properties):
    ...
    for prop in properties:
        _label = ""
        _label_o = ""
        if not prop['p']['value'] in _agrouped[canonical]:
            _agrouped[canonical][prop['p']['value']] = []
        if "label_o" in prop: _label_o = prop['label_o']['value']
        _agrouped[canonical][prop['p']['value']].append([prop['o']['value'], prop['label']['value'], prop['prov']['value'], _label_o])
    agrouped = verifica_valores_divergentes(_agrouped, canonical)
    return agrouped
